...
export const CartProvider = ({ children }) => {
  ...
  const updateQuantity = (productId, quantity) => {
    ...
    else setCartItems(prevItems => prevItems.map(item => item.id === productId ? { ...item, quantity } : item))
  }
  const clearCart = () => setCartItems([])
  const getTotalPrice = () => cartItems.reduce((total, item) => total + parseFloat(item.price) * item.quantity, 0)
  return (
